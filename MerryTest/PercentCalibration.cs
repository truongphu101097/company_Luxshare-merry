...
namespace MerryTest
{
    public partial class PercentCalibration : Form
    {
        ...
        string RF_JZ = "RF_JZ";
        public PercentCalibration(List<string> addRF)
        {
            InitializeComponent();
            //TestValue = Test;
            AddRF = addRF;
        ...
        private void RFCalibration_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                btnYes_Click_1(null, null);
            }
            else if (e.KeyCode == Keys.N)
            {
                btnNo_Click_1(null, null);
            }
        }
            ...
            try
            {
                foreach (var item in AddRF)
                {
                    ...
                    if (add[2].Contains(SectionN9320b))
                    {
                        ...
                        string value = INIGetStringValue(path, SectionN9320b, add[0], "0");
                        string[] PR = value.Split('%');
                        string mul = PR[0];
                        if (Convert.ToDouble(mul) == 0)
                        {
                            RealValue[$"{add[0]}&{RF_JZ}"] = (Convert.ToDouble(add[1])).ToString();
                        }
                        else
                        {
                            RealValue[$"{add[0]}&{RF_JZ}"] = (Convert.ToDouble(add[1]) / Convert.ToDouble(PR[0]) * 100).ToString();
                        }
                    }
                    if (add[2].Contains(SectionRT550))
                    {
                        string value = INIGetStringValue(path, SectionRT550, add[0], "0");
                        string[] PR = value.Split('%');
                        string mul = PR[0];
                        if (Convert.ToDouble(mul) == 0)
                        {
                    ...
                    {
                        string value = INIGetStringValue(path, MT8852B, add[0], "0");
                        string[] PR = value.Split('%');
                        string mul = PR[0];
                        if (Convert.ToDouble(mul) == 0)
                        {
                    ...
                    {
                        string value = INIGetStringValue(path, HanOpticSens, add[0], "0");
                        string[] PR = value.Split('%');
                        string mul = PR[0];
                        if (Convert.ToDouble(mul) == 0)
                        {
            ...
            try
            {
                foreach (var item1 in AddRF)
                {
                    string[] item = item1.Split('*');//item[0]:测试项目名称   item[1]:测试显示值     item[2]测试指令
                                                     ...
                                                     //如果测试命令包含（N9320B，MT8852B, RT550, HanOpticSens, Percent）
                    if (item[2].Contains(SectionN9320b) || item[2].Contains(MT8852B) || item[2].Contains(SectionRT550) || item[2].Contains(HanOpticSens))
                    {
                        writekey.Add($"{item[0]}&{RF_JZ}");
                    ...
                    if (item[2].Contains(SectionN9320b))
                    {
                        ...
                        if (!key.Contains(item[0])) continue;
                        string value = INIGetStringValue(path, SectionN9320b, item[0], "0");
                        string[] PR = value.Split('%');
                        string mul = PR[0];
                        if (Convert.ToDouble(mul) == 0)
                        {
                            double originalValue = Convert.ToDouble(INIGetStringValue(path2, RF_JZ, item[0], "0")) / Convert.ToDouble(item[1]) * 100;
                            double roundedValue = (int)originalValue;

                            CalibrationValue[$"{item[0]}&{HanOpticSens}"] = (roundedValue + "%").ToString();
                        }
                        else
                        {
                            double originalValue = Convert.ToDouble(INIGetStringValue(path2, RF_JZ, item[0], "0")) / Convert.ToDouble(item[1]) * 100 * Convert.ToDouble(mul) / 100;
                            double roundedValue = (int)originalValue;
                    ...
                    if (item[2].Contains(MT8852B))
                    {
                        ...
                        if (!key.Contains(item[0])) continue;
                        string value = INIGetStringValue(path, MT8852B, item[0], "0");
                        string[] PR = value.Split('%');
                        string mul = PR[0];
                        if (Convert.ToDouble(mul) == 0)
                        {
                    ...
                    if (item[2].Contains(SectionRT550))
                    {
                        ...
                        if (!key.Contains(item[0])) continue;
                        string value = INIGetStringValue(path, SectionRT550, item[0], "0");
                        string[] PR = value.Split('%');
                        string mul = PR[0];
                        if (Convert.ToDouble(mul) == 0)
                        {
                    ...
                    if (item[2].Contains(HanOpticSens))
                    {
                        ...
                        if (!key.Contains(item[0])) continue;
                        string value = INIGetStringValue(path, HanOpticSens, item[0], "0");
                        string[] PR = value.Split('%');
                        string mul = PR[0];
                        if (Convert.ToDouble(mul) == 0)
                        {
                ...
                foreach (var item in writekey)
                {
                    ...
                    {
                        string value = CalibrationValue[item];
                        Calibration=value.Split('%');

                        if (Math.Abs(double.Parse(Calibration[0])) > double.Parse(txbLimitCompensation.Text))
                        {
                ...
                foreach (var item in INIGetAllSectionNames(path))
                {
                    ...
                    if (item != SectionN9320b && item != SectionRT550 && item != HanOpticSens && item != MT8852B) INIDeleteSection(path, item);
        ...
        public void Compensation()
        {
            ...
            foreach (var item1 in AddRF)
            {
                ...
                try
                {
                    value = Convert.ToDouble(INIGetStringValue(path2, RF_JZ, item[0], "0")) / Convert.ToDouble(INIGetStringValue(path, RF_JZ, item[0], "0"))*100;
                }
                catch (Exception ex)
                {
                    value = 0;
        ...
        private void PercentCalibration_Load(object sender, EventArgs e)
        {
